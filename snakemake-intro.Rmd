...
rule annotate_genome:
    ...
    threads: 8
    shell: 
        "prokka --force --prefix {wildacards.sample} "
...
5. Provides a GBK file to prokka's `--proteins` argument via `--config` or `--configfile`
---
...
- Consider setting `--latency-wait` with larger number (default: `5`)
