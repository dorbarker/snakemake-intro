...
"conda create -n smk-lesson-1 -c conda-forge -c bioconda prokka pirate"
```
...
4. Mixing in Python (and R and Julia)
5. Assignment 2
...
3. Write a rule with a `run` block that reads `PIRATE.gene_families.tsv` and creates a multifasta for each with all the alleles of the gene 
    - columns of interest: `gene_family` & `number_genomes`

4. A rule that:
    - Uses an input function to selects the multifastas present in 100% of genomes
...
```python
import pandas as pd
data_table = pd.read_csv("path/to/file.tsv", sep = "\t")
# select values from columnA where columnC is greater than 42
selector_values = data_table["columnC"] > 42
result = data_table["columnA"].loc[selector_values]
```
